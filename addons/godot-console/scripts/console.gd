...
func _init() -> void:
	# Built-in methods:
	create_command("targetFPS", Engine.set_max_fps, "set the desired frames per second. A value of 0 means no limit.")
	create_command("physicsTicks", Engine.set_physics_ticks_per_second, "set physics ticks per second.")
	# Custom methods:
	create_command("clear", clear, "clear the console history.")
	create_command("help", _command_help, "show all console commands.")
